...
const ChargingStationSearchMap = ({ temporaryArray, myLoc }) => {
  ...
  useEffect(() => {
    if (myLoc) { // myLoc이 null이 아닐 때만 initMap 호출
      initMap();
    }
  ...
  const handleResetMap = () => {
    map.current.setCenter(
      new window.kakao.maps.LatLng(myLoc.lat, myLoc.lng)
    );
    map.current.setLevel(3);
  };
  ...
  return (
    <div style={{ position: "relative" }}> {/* 부모 요소를 relative로 설정 */}
      <div
        id="map"
        style={{
          margin: "20px",
      ...
      />
      <Button
        style={{ position: "absolute", zIndex: "2", bottom: "10px", right: "40px" }}
        variant="contained"
        color="primary"
